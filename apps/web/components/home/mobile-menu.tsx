...
export function MobileMenu() {
  ...
  // The overlay+panel must be portaled to <body>: the parent <nav>
  // applies `backdrop-blur-xl`, which in CSS creates a new containing
  // block for descendants with `position: fixed`. Without the portal
  // the panel only spans the height of the navbar (~64px) and the
  // menu items render clipped to the navbar strip.
    ...
    ? createPortal(
        <>
          ...
          <div
            ...
            className={
              'fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm transition-opacity duration-200 lg:hidden ' +
          ...
          <div
            className={
              'fixed inset-y-0 right-0 z-[70] flex w-80 max-w-[85vw] flex-col overflow-y-auto bg-white px-6 pt-20 pb-8 shadow-2xl transition-transform duration-300 lg:hidden ' +
