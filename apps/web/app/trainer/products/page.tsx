...
function StatusBadge({ p }: { p: TrainerProduct }) {
  if (p.published) return <Badge variant="success">Publicado</Badge>;
    ...
    <div className="space-y-5">
      ...
      {q.isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center">
          ...
          <Link href="/trainer/products/new" className="mt-4 inline-block">
            <Button size="sm">
              <Plus className="h-4 w-4" />
              Crear rutina
            </Button>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          {items.map((p) => (
            <button
              ...
              className="group flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-left transition-all hover:border-brand-orange/40 hover:bg-white/[0.04]"
            >
              {p.cover_url ? (
                ...
                <div className="flex h-36 w-full items-center justify-center rounded-lg bg-white/[0.04] text-xs text-white/30">
                  Sin portada
                </div>
              )}
